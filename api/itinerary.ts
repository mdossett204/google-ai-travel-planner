...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    - Transportation preferences should shape the plan. Favor compact, locally explorable areas and practical movement on foot, transit, or short rides unless longer travel is clearly justified.
    - If the stated transportation preference is a poor fit for the destination, quietly adapt the daily movement style to the most practical local option without changing the overall trip character.
    ...
    - If a must-see location is provided, include it only if it is actually in the requested destination and fits naturally into the daily geography.
    ...
    EXAMPLE OUTPUT
    DAY 1
    Area Anchor: French Quarter
    Morning: Walk Jackson Square and the surrounding historic streets | Area: French Quarter | Pace: easy | Why it fits: easy first-day orientation
    Afternoon: Visit the New Orleans Jazz Museum or a nearby cultural site | Area: French Quarter | Pace: moderate | Why it fits: one anchor cultural stop
    Evening: Stroll the Moon Walk riverfront promenade at sunset | Area: French Quarter | Pace: easy | Why it fits: light scenic wind-down
    Daily Flow Note: This sequence stays compact in the French Quarter and shifts naturally from neighborhood orientation to culture to a relaxed riverfront evening.
